...
def load_data(feeding_file, harvest_file, sampling_file, transfer_file=None):
    ...
    feeding = normalize_columns(feeding)
    harvest = normalize_columns(harvest)
    sampling = normalize_columns(sampling)

    transfers = None
    if transfer_file:
        transfers = pd.read_excel(transfer_file)
        transfers = normalize_columns(transfers)

        # Map transfers to expected names
        if 'NUMBER OF FISH' in transfers.columns:
            transfers.rename(columns={'NUMBER OF FISH':'NUM_FISH'}, inplace=True)
        if 'AVERAGE BODY WEIGHT (g)' in transfers.columns:
            transfers.rename(columns={'AVERAGE BODY WEIGHT (g)':'ABW_G'}, inplace=True)
        transfers['DATE'] = pd.to_datetime(transfers['DATE'])
        transfers['TOTAL_WEIGHT_KG'] = transfers['NUM_FISH'] * transfers['ABW_G'] / 1000

    return feeding, harvest, sampling, transfers

# -------------------------------
# 3. Preprocess Cage 2 with Transfers
# -------------------------------
def preprocess_cage2(feeding, harvest, sampling, transfers=None):
    ...
    if transfers is not None and not transfers.empty:
        cage_transfers = transfers[(transfers.get('CAGE_FROM', -1) == cage_number) | (transfers.get('CAGE_TO', -1) == cage_number)]
        cage_transfers = cage_transfers.sort_values('DATE')

        for _, row in cage_transfers.iterrows():
            date = row['DATE']
            if row.get('CAGE_FROM', -1) == cage_number:
                mask = sampling_c2['DATE'] >= date
                sampling_c2.loc[mask, 'NUMBER OF FISH'] = (sampling_c2.loc[mask, 'NUMBER OF FISH'] - row['NUM_FISH']).clip(lower=0)
            if row.get('CAGE_TO', -1) == cage_number:
                mask = sampling_c2['DATE'] >= date
                sampling_c2.loc[mask, 'NUMBER OF FISH'] += row['NUM_FISH']

        # Recalculate biomass after transfers
        sampling_c2['TOTAL_WEIGHT_KG'] = sampling_c2['NUMBER OF FISH'] * sampling_c2['AVERAGE BODY WEIGHT (g)'] / 1000
