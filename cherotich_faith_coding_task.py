# app.py — Fish Cage Production Analysis (with Transfers, Missing Values Handled)
...
def load_data(feeding_file, harvest_file, sampling_file, transfer_file=None):
    ...
    # Parse dates
    for df in [feeding, harvest, sampling] + ([transfers] if transfers is not None and not transfers.empty else []):
...
def preprocess_cage2(feeding, harvest, sampling, transfers=None):
    ...
    # If transfers exist, adjust stocking ABW
    if transfers is not None and not transfers.empty:
        t = _clip(transfers)
        t_in = t[t.get("DESTINATION CAGE", -1) == cage_number].sort_values("DATE") if "DESTINATION CAGE" in t.columns else pd.DataFrame()
        if not t_in.empty:
            first = t_in.iloc[0]
...
def compute_summary(feeding_c2, sampling_c2):
    ...
    # eFCR
    growth_cum = summary["ΔBIOMASS_STANDING"].cumsum(skipna=True)
    summary["PERIOD_eFCR"]     = np.where(summary["ΔBIOMASS_STANDING"]>0, summary["FEED_PERIOD_KG"]/summary["ΔBIOMASS_STANDING"], np.nan)
    summary["AGGREGATED_eFCR"] = np.where(growth_cum>0, summary["FEED_AGG_KG"]/growth_cum, np.nan)

    first_idx = summary.index.min()
    summary.loc[first_idx, ["FEED_PERIOD_KG","ΔBIOMASS_STANDING","PERIOD_eFCR"]] = np.nan
    return summary
