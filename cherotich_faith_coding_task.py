...
# Cage 2 Preprocessing – Simplified ABW & Stocking
# =================================================
def preprocess_cage2(feeding, harvest, sampling, transfers=None):
    """Preprocess Cage 2 data with simplified stocking & ABW calculation"""
    ...
    # ----------------------------
    # Determine initial stocking & ABW
    # ----------------------------
    stocked_fish = 7290
    initial_abw_g = 11.9

    if transfers is not None and not transfers.empty:
        t_in = _clip(transfers)
        t_in = t_in[t_in.get("DESTINATION CAGE", 0) == cage_number].sort_values("DATE")
        if not t_in.empty:
            first = t_in.iloc[0]
            stocked_fish = int(float(first.get("NUMBER OF FISH", stocked_fish)))
            if pd.notna(first.get("TOTAL WEIGHT [KG]")) and stocked_fish:
    ...
    # ----------------------------
    base = pd.concat([
        pd.DataFrame([{"DATE": start_date, "CAGE NUMBER": cage_number, "AVERAGE BODY WEIGHT(G)": initial_abw_g}]),
        sampling_c2
    ], ignore_index=True).sort_values("DATE").reset_index(drop=True)
    base["STOCKED"] = stocked_fish

    # ----------------------------
    # Ensure final harvest ABW is included
    # ----------------------------
    if not harvest_c2.empty:
        final_date = harvest_c2["DATE"].max()
        if not (base["DATE"] == final_date).any():
            hh = harvest_c2[harvest_c2["DATE"] == final_date].copy()
            ...
            if np.isnan(abw_final) and abw_colh:
    ...
    if (h_fish_col or h_kg_col) and not harvest_c2.empty:
        ...
        h["HARV_FISH_CUM"] = pd.to_numeric(h[h_fish_col], errors="coerce").fillna(0).cumsum() if h_fish_col else 0
        h["HARV_KG_CUM"] = pd.to_numeric(h[h_kg_col], errors="coerce").fillna(0).cumsum() if h_kg_col else 0
        cum_dfs.append(h[["DATE", "HARV_FISH_CUM", "HARV_KG_CUM"]])
    ...
    if transfers is not None and not transfers.empty:
        ...
        if origin_col and dest_col:
            ...
            t["ORIGIN_INT"] = t[origin_col].apply(lambda x: int(re.search(r"(\d+)", str(x)).group(1)) if pd.notna(x) and re.search(r"(\d+)", str(x)) else None)
            t["DEST_INT"] = t[dest_col].apply(lambda x: int(re.search(r"(\d+)", str(x)).group(1)) if pd.notna(x) and re.search(r"(\d+)", str(x)) else None)
            ...
            if not tout.empty:
                ...
                cum_dfs.append(tout[["DATE", "OUT_FISH_CUM", "OUT_KG_CUM"]])
            ...
            if not tin.empty:
                ...
                cum_dfs.append(tin[["DATE", "IN_FISH_CUM", "IN_KG_CUM"]])

    # Merge cumulatives once
    if cum_dfs:
        cumulative_df = cum_dfs[0]
        for df in cum_dfs[1:]:
            cumulative_df = pd.merge_asof(cumulative_df.sort_values("DATE"),
                                          df.sort_values("DATE"),
                                          on="DATE",
                                          direction="backward")
        base = pd.merge_asof(base.sort_values("DATE"),
                             cumulative_df.sort_values("DATE"),
                             on="DATE",
                             direction="backward")
        for col in ["HARV_FISH_CUM", "HARV_KG_CUM", "IN_FISH_CUM", "IN_KG_CUM", "OUT_FISH_CUM", "OUT_KG_CUM"]:
